...
Object.defineProperties(_wcl, {
  ...
  capitalize: {
    ...
    enumerable: true,
    value: function(str) {
      return str.replace(/^[a-z]{1}/, function($1) {
  ...
  isCSSPropertySupport: {
    ...
    value: function(property) {
      ...
      property = /^-ms/.test(property)
        ? 'ms' + this.camelCase(property.replace(/-ms/, ''))
        : this.camelCase(property);
  ...
  scrollLock: {
    ...
    value: function(isLock = true, eventLock = this.grabEventLock()) {
      const overScrollSupport = this.isCSSPropertySupport(
        'overscroll-behavior'
      );
  ...
  rollTo: {
    ...
    value: function(y, offset = 0) {
      ...
      if (
        typeof y.nodeType !== 'undefined' &&
        y.nodeType === 1 &&
        typeof y.getBoundingClientRect === 'function'
      ) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const scroll = () => {
          ...
          if (
            this.scrollY < 0 ||
            this.scrollY + winHeight >= height ||
            shift === 0 ||
            Math.abs(this.scrollY - y) <= Math.abs(shift)
          ) {
            this.scrollY = y;
            resolve();
            return;
  ...
  pursuer: {
    ...
    value: function() {
      ...
      if (this.isEventSupport('touchstart')) {
        ...
        typeof navigator.msPointerEnabled != 'undefined' &&
  ...
  addStylesheetRules: {
    ...
    value: function(
      ...
      propStr = Object.keys(props)
        ...
        .join(sign);
      findIndex = Array.from(styleSheet.cssRules).findIndex(
